<template>
  <div class="app-container" style="height: calc(100vh - 50px)">
    ...
    >
      ...
      <el-table-column prop="normalWorkHour" label="正常工作小时" width="100px">
        <template scope="scope">
          <el-input
            size="small"
      ...
      <el-table-column
        ...
        label="小计"
        width="60px"
        :formatter="subtotalWorkHourFormatter"
      >
      </el-table-column>
...
export default {
  ...
  mounted() {
    this.form.yearMonth = this.yearMonth != null ? this.yearMonth : new Date();
    this.formDefaultValue();
  ...
  watch: {
    ...
    selectId: function (newselectId, oldselectId) {
      this.formDefaultValue();
      this.search();
    },
  },
  methods: {
    ...
    formDefaultValue() {
      this.listLoading = true;
      getDefaultParams({ name: this.ajaxName, userId: this.selectId }).then(
        (response) => {
